...
int main(void) {
	...
	double totalWet = 0, totalDry = 0, ratio = 0;
	...
	*/
	// Add one line here! 
	totalWet = *((*cookie)) + *((*cookie) + 1); // *((*cookie)) + *((*cookie)+1) OR cookie[0][0] + cookie[0][1]
	...
	*/
	// Add one line here! 
	totalDry = *(*(cookie + 1)) + *(*(cookie + 1)+ 1) + *(*(cookie + 1) + 2); // *((*cookie)) + *((*cookie)+1) OR cookie[1][0] + cookie[1][1] + cookie[1][2]
